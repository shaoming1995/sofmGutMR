...
#' @param GWASsummay 本地预处理好的暴露GWAS summay
#' @param pval 阈值
#' @param expname 暴露的名称
#' @export
Local_Gut<-function(name,key,savefile,PATH,GWASsummay,pval,expname){
  A<-name
  ...
  exp<-subset(GWASsummay,pval.exposure<pval)
  if(C==2310000){
    for (i in filename[,1]){
    ...
    write.csv(C_temp,Cname,row.names = F)
      ...
      cat("当前分析已全部完成！请前往",savefile,"文件夹下查看结果")}else{
