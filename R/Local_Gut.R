...
#' @param GWASIV 本地预处理好的暴露IV
#' @param expname 暴露的名称
#' @export
Local_Gut<-function(name,key,savefile,PATH,GWASIV,expname){
  A<-name
  ...
  if(C==2310000){
    for (i in 1:216){
      ...
      test<-try(OUT1<-extract_outcome_data(snps=GWASIV$SNP,outcomes=OUT_temp[i,1],proxies=T,maf_threshold = 0.01,access_token = NULL))
      ...
      if(class(test)!="NULL"){
        ...
        #去除回文
    dat<-harmonise_data(exposure_dat=GWASIV,outcome_dat=OUT1,action=2)
    res <- mr(dat)
